...
export default function AddLicenseModal({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      fetch(
        `${process.env.REACT_APP_API_URL}/businesses/search?query=${encodeURIComponent(query)}&limit=10`,
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      fetch(
        `${process.env.REACT_APP_API_URL}/addresses/search?query=${encodeURIComponent(query)}&limit=10`,
        { signal: controller.signal }
      )
        .then(async (res) => {
          if (!res.ok) {
      ...
      <form id={formId} onSubmit={handleSubmit} className="space-y-4">
        ...
        {licenseType === "1" ? (
          <div>
            ...
            {selectedBusiness && (
              <p className="mt-2 text-xs text-gray-600">
                Selected business: {selectedBusiness.name?.trim() || selectedBusiness.trading_as?.trim() || `Business #${selectedBusiness.id}`}
              </p>
            )}
          </div>
        ) : (
          <div>
            ...
            {selectedAddress && (
              <p className="mt-2 text-xs text-gray-600">
                Selected address: {selectedAddress.combadd || `Address #${selectedAddress.id}`}
              </p>
            )}
          </div>
        )}
        ...
        <div className="rounded border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600">
          <p>
            Fiscal Year (preview): <span className="font-medium">{fiscalYear}</span>
          </p>
          <p>
            Expiration (preview): <span className="font-medium">{expirationDate}</span>
          </p>
